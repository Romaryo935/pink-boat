...
// which generates every polyline between start and finish
  ...
  Widget build(BuildContext context) {
   ...
   dynamic hospitalInstance = ModalRoute.of(context).settings.arguments;
    setState(() {
      hospitalItemB =hospitalInstance;
    });
    
   double a4;
    double b4;
       if (hospitalItemB["nearestSchedule"] != false) {
      a4 = hospitalItemB["nearestSchedule"]["lat"] ;
      b4 = hospitalItemB["nearestSchedule"]["lng"] ;
                  ...
                  children: <Widget>[
                    Stack(
                      children: <Widget>[
                        Container(
                          child: Image.network(
                              hospitalItemB["mobileCenter"]["test_center"]
                                  ["medical_org"]["pic_url"],
                              fit: BoxFit.fitWidth),
  ...
  _detailItem(BuildContext context, initialLocation) {
     var number = hospitalItemB["mobileCenter"]["test_center"]["medical_org"]
        ["phone"]["phone"];
    ...
    if (hospitalItemB["nearestSchedule"] != false) {
      a = hospitalItemB["nearestSchedule"]["lat"] ?? 'lolololololo';
      b = hospitalItemB["nearestSchedule"]["lng"] ?? 'lolololololo';
      c = hospitalItemB["nearestSchedule"]["from"] ?? 'lolololololo';
      x = hospitalItemB["nearestSchedule"]["to"] ?? 'lolololololo';

      d = hospitalItemB["nearestSchedule"]["street_id"]["name"] ??
          'lolololololo';
      e = hospitalItemB["nearestSchedule"]["street_id"]["area"]["name"] ??
          'lolololololo';
      f = hospitalItemB["nearestSchedule"]["street_id"]["area"]["city"]
              ["name"] ??
          'lolololololo';
    } else {
      a = 30.23265556;
                ...
                Container(
                  ...
                  children: <Widget>[
                    ListTile(
                      ...
                      title: Text("location"),
                      subtitle: Text("Lat: " +
                              // hospitalItemB["nearestSchedule"]["lat"].toString()
                              a .toString()+
                              "      Long: " +
                              b.toString()
                          // hospitalItemB["nearestSchedule"]["lng"].toString()
                          ),
                    ),
                    SizedBox(
                      height: 400,
                    ...
                    ListTile(
                      ...
                      title: Text("Phone"),
                      subtitle: Text(hospitalItemB["mobileCenter"]
                              ["test_center"]["medical_org"]["phone"]["phone"]
                          .toString()), //
                    ),
                      new SizedBox(
                      width: 300.0,
                    ...
                    ListTile(
                      ...
                      subtitle: Text(
                        d + e + f,
                        //  hospitalItemB["nearestSchedule"]["street_id"]["name"]+", "+hospitalItemB["nearestSchedule"]["street_id"]["area"]["name"]+", "+hospitalItemB["nearestSchedule"]["street_id"]["area"]["city"]["name"]  ?? 'lolololo',
                      ),
                    ),
                    new SizedBox(
                      width: 300.0,
